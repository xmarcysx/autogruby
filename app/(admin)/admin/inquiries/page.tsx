...
const STATUS_STYLES: Record<string, string> = {
  ...
  read: 'bg-slate-700 text-slate-300 border-slate-600',
  ...
  return (
    <AdminShell>
      <div className="p-6 lg:p-8 space-y-6">
        <div>
          <h1 className="text-2xl font-black text-white">Zapytania klientów</h1>
        ...
        {inquiries.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-16 bg-slate-900 rounded-2xl border border-slate-800">
            ...
            <p className="text-slate-400 font-medium">Brak zapytań</p>
            <p className="text-slate-600 text-sm mt-1">Zapytania od klientów pojawią się tutaj</p>
          </div>
        ) : (
          <div className="space-y-3">
            {inquiries.map((inquiry) => (
              <div
                key={inquiry.id}
                className="bg-slate-900 rounded-2xl border border-slate-800 p-5 hover:border-slate-700 transition-colors"
              >
                <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-3 mb-2">
                      <span className="text-white font-semibold text-sm">{inquiry.name}</span>
