      ...
      <div className="container relative z-10">
        ...
        <div className="mt-12 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
          {MOCK_TESTIMONIALS.map((testimonial) => (
            <article
              key={testimonial.id}
              className="bg-sky-700/60 backdrop-blur-sm rounded-xl border border-sky-500/60 p-5 flex flex-col hover:border-brand-gold/40 hover:bg-sky-700/80 transition-all duration-300"
            >
              {/* Stars */}
              <div className="flex gap-0.5 mb-3" aria-label={`Ocena: ${testimonial.rating} na 5`}>
                {Array.from({ length: 5 }).map((_, i) => (
                  <Star
                    key={i}
                    className={`h-4 w-4 ${i < testimonial.rating ? 'fill-brand-gold text-brand-gold' : 'text-sky-500'}`}
                    aria-hidden="true"
                  />
                ))}
              </div>
              ...
              {/* Author */}
              <div className="mt-4 pt-4 border-t border-sky-500/60">
                ...
                <time
                  ...
                  className="text-xs text-sky-300 mt-0.5 block"
                >
